...
{
	//Game
	m_PlayerShip.Initialise(Vector2(SCREEN_WIDTH - 250, SCREEN_HEIGHT /2), VK_UP, VK_DOWN);

	m_EnemyArmy.Initialise(this);

	m_Missile.Initialise(Vector2(m_PlayerShip.GetPosition().x + (m_PlayerShip.GetSize().x / 0.5f), m_PlayerShip.GetPosition().y / 2), Vector2(MISSILE_SPEED, 0));
...
{
	if (m_EnemyArmy.EnemiesRemaining() <= 0)
	{
		//player wins
		m_GameState = E_GAME_STATE_WIN_GAME;
	}
	else if (m_EnemyArmy.Landed())
	{
