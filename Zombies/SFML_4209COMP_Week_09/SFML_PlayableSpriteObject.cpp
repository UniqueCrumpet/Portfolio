...
SFML_BulletSpriteObject* SFML_PlayableSpriteObject::processEvents(float elapsedTime, sf::Transform transform, sf::Vector2f mouseScreenPosition)
{
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::W))//replace with w 
	{
	...
	else if (sf::Keyboard::isKeyPressed(sf::Keyboard::S))//Replace with s
	{
		float moveAngle = faceAngle + 90;
		moveAngle = moveAngle / 180 * PI;
		float cosine = cos(moveAngle);
	...
	else if (sf::Keyboard::isKeyPressed(sf::Keyboard::D))//Replace with d
	{
	...
	else if (sf::Keyboard::isKeyPressed(sf::Keyboard::A))//replace with a
	{
